...
if (keepAll)
    ...
    if ~isempty(allLeftNodes)
        ...
        %    [leftCoords,ipIndices,keepLeft] = intersectCols(uint16(ipCoords),uint16(allLeftNodes([3 2 1],:)));
        %else
    ...
    if ~isempty(allRightNodes)
        ...
        %        intersectCols(uint16(ipCoords), uint16(allRightNodes([3 2 1],:)));
        %else
        [rightCoords ipIndices keepRight] = intersectCols(uint16(ipCoords), uint16(allRightNodes([2 1 3],:)));
        %end
